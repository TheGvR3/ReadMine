...
function UpdateOpera() {
  ...
  // Form: solo campi opera-level (post-migration 003)
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        // Usa /full per avere autori/generi/serie già aggregati
        ...
        if (resOpera?.ok) {
          ...
          if (data.autori_ids && data.autori) {
            const nomi = data.autori.split(", ");
            setSelectedAutori(
              data.autori_ids.map((id, i) => ({ value: id, label: nomi[i] || `Autore #${id}` }))
            );
          }
          if (data.generi_ids && data.generi) {
            const nomi = data.generi.split(", ");
            setSelectedGeneri(
              data.generi_ids.map((id, i) => ({ value: id, label: nomi[i] || `Genere #${id}` }))
            );
          }
  ...
  const loadAutoriOptions = async (input) => {
    if (!input) return [];
  ...
  const loadGeneriOptions = async (input) => {
    if (!input) return [];
    const r = await secureFetch(
  ...
  const loadSerieOptions = async (input) => {
    if (!input) return [];
    const r = await secureFetch(
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSend = {
      ...
      stato_opera: formData.stato_opera,
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none font-bold text-gray-700"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  name="descrizione_opera"
                  value={formData.descrizione_opera}
                  onChange={handleChange}
                  rows={4}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-bold text-gray-700"
                />
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-700">
                ℹ️ Editore, ISBN e anno di pubblicazione sono dati di <strong>edizione</strong> —
                gestiti separatamente sull'opera dopo questo salvataggio.
              </div>
